...
void Scene::execute() {
	...
	//// TODO : Create save image to model controller
	//SaveImage2Model saveImage(m_context["output_buffer"]->getBuffer());
}
...
void Scene::setupLights() {
	...
	if(!model.lighting.empty()) {
		std::vector<Light> json_lights = model.lighting.lights;
		n=json_lights.size(); //number of light sources... comes from JSON
		brightness = model.lighting.totalBrightness;
...
std::string Scene::texpath( const std::string& base )
{
